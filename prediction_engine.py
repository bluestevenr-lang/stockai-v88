...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
功能：
  1. 量权价格参照：VWAP (成交量加权平均价) - 观察日线量价关系
...
class InstitutionalPredictor:
    ...
    def calculate_vwap(self, window: int = 20) -> pd.Series:
        ...
        日线HLC3按成交量加权的滚动均值，是价格观察参照。
        没有投资者身份或成交持仓记录，不能推断机构成本及盈亏。
        ...
        try:
            ...
            typical_price = typical_price.where(np.isfinite(typical_price) & (typical_price > 0))
            volume = self.df['Volume'].where(np.isfinite(self.df['Volume']) & (self.df['Volume'] >= 0))

            # 成交量加权
            denominator = volume.rolling(window=window).sum()
            vwap = (typical_price * volume).rolling(window=window).sum() / denominator.where(denominator > 0)
    ...
    def calculate_alpha_factors(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if pd.notna(current_vwap) and current_vwap > 0:
                ...
                # VWAP信号
                if vwap_deviation > 5:
                    factors['vwap_signal'] = "↑ 价格高于量权均值5%以上"
                elif vwap_deviation > 0:
                    factors['vwap_signal'] = "↑ 价格略高于量权均值"
                elif vwap_deviation > -5:
                    factors['vwap_signal'] = "↓ 价格略低于量权均值"
                else:
                    factors['vwap_signal'] = "↓ 价格低于量权均值5%以上"
            else:
                factors['vwap_20'] = None
                factors['vwap_deviation'] = None
                factors['vwap_signal'] = "○ VWAP窗口数据不足"
            ...
            if len(self.df) >= 20:
                ...
                if len(bb_width_series) >= 120:
                    bb_percentile = (bb_width_series.tail(120) <= bb_width_series.iloc[-1]).sum() / 120 * 100

                    if bb_percentile < 20:
                        factors['bb_squeeze'] = "↔ 波动收缩（近120条低分位，方向待确认）"
                    elif bb_percentile < 40:
                        factors['bb_squeeze'] = "🟡 轻度挤压（波动率偏低）"
                    else:
                        factors['bb_squeeze'] = "↔ 波动未落入低分位"
                else:
            ...
            # 4. 量价背离
            if (len(self.df) >= 6 and self.df['Close'].iloc[-6] > 0
                    and self.df['Volume'].iloc[-6] > 0
                    and np.isfinite(self.df['Volume'].iloc[-1]) and self.df['Volume'].iloc[-1] >= 0):
                price_change_5d = ((self.df['Close'].iloc[-1] - self.df['Close'].iloc[-6]) /
                                   self.df['Close'].iloc[-6] * 100)
                volume_change_5d = ((self.df['Volume'].iloc[-1] - self.df['Volume'].iloc[-6]) /
                                    self.df['Volume'].iloc[-6] * 100)
    ...
    def calculate_risk_engine(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 日涨跌频率不等于已结算策略胜率，不据此分配真实仓位。
            risk['kelly_position'] = None
            risk['position_basis'] = '缺少同策略合格结算样本；仓位仅按中央风险预算与原失效条件计算'

            # 3. 风险评级
            if risk['stop_loss_pct']:
                if risk['stop_loss_pct'] < 3:
                    risk['risk_grade'] = "ATR距离 <3%"
                elif risk['stop_loss_pct'] < 5:
                    risk['risk_grade'] = "ATR距离 3–5%"
                elif risk['stop_loss_pct'] < 8:
                    risk['risk_grade'] = "ATR距离 5–8%"
                else:
                    risk['risk_grade'] = "ATR距离 ≥8%"
            else:
                risk['risk_grade'] = "未评级"
    ...
    def call_gemini_oracle(self, gemini_api_key: str, model_name: str = "gpt-6-astra") -> Dict[str, Any]:
        """
...
"""
    ...
    def calculate_chandelier_exit(self, atr_period: int = 22, atr_multiplier: float = 3.0) -> Dict[str, Any]:
        ...
        try:
            ...
            if result['ce_long_latest'] > 0 and current_price < result['ce_long_latest']:
                result['signal'] = '↓ 低于多头波动参照；核对中央原失效条件'
            elif result['ce_short_latest'] > 0 and current_price > result['ce_short_latest']:
                result['signal'] = '↑ 高于空头波动参照；趋势反转仍待确认'
            else:
                result['signal'] = '↔ 位于波动参照之间；不代表安全或持有许可'
    ...
    def call_gemini_pre_mortem(self, gemini_api_key: str, model_name: str = "gpt-6-astra",
...
"""
    ...
    def call_gemini_entry_advisor(self, gemini_api_key: str, entry_price: float, entry_date: str,
                                    candle_data: dict = None, model_name: str = "gpt-6-astra",
                                    macro_context: dict = None) -> Dict[str, Any]:
        """
