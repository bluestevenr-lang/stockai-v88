...
11. 执行 `RESTORE_V88.bat -Stage Verify -MacReceiverDisabled -PhoneAnswerConfirmed -RemoteMacReadConfirmed`。
    ...
    Runtime 预检若仍出现 `MANUAL_REQUIRED`，只允许复用 Prepare 时同一报告中已经逐项确认、且证据整行完全
    未变化的项目；新增、未知、重复、数据变化、旧报告哈希变化或未来时间报告都会硬停。不存在“全部放行”。
    Verify 失败时可能已开始永久 Gateway 转换，必须实查任务、监听和飞书状态，不能根据一条报错假定 Win 已关闭。
...
- K3 未通过 managed OAuth 与真实哨兵时，禁止把正式 V88-GPT 飞书接收端切到 Win；Mac 继续临时接待，且不得冒充双审完成。
