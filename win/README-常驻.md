...
| `install_openclaw_win.ps1` | 上面的 bat | GPT 方案 B：装 Node/OpenClaw/Codex/飞书插件，生成脱敏只读工作区，注册开机自启网关；不写密钥，不启 Kimi API |
| `启用OpenClaw飞书-双击我.bat` | 人，验收后一次性 | **先停 Mac 网关**，再在 Win 本机输入飞书 App ID/Secret，绑定 `v88-mobile` 并切换飞书主机 |
| `openclaw-v88/` | 同步任务 + 安装器 | 只读代理规则与脱敏投影器；每轮双仓拉取后刷新，禁止复制账户/资产/持仓数量/成本 |

## 注册了哪两个任务

- **`V88-遥控常驻`** — 开机 +1 分钟起；不管用户是否登录都运行；失败每 5 分钟重试，最多 3 次；运行时长不限
- **`V88-夜间重启遥控`** — 每天 **03:30**（撞 V88 维护时间窗 03:00-04:30）；可唤醒计算机
- **`OpenClaw Gateway`** — 开机 +2 分钟起；S4U 无需登录；失败每 5 分钟重试，最多 12 次
...
- **飞书只能有一个 OpenClaw 主机**：Win 验收前保持飞书未配置；先停 Mac Gateway，再运行 Win 的飞书切换脚本。
