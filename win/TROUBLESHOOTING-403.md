...
> 别再宣布「已解决」——今晚第四次了。
>
> **勘误 5（2026-07-30 22:47）—— 「环境 ID 被污染 / stale lease」假设，已用受控实验证伪。**
> 假设内容：`env_01A99j…` 在 17:30 删会话时被服务端标记，之后复用它必死；
> 换新环境就活。修法（codex 复核也指向同一修法）：不复用、每轮新铸。
>
> **实验（22:39–22:43）**：备份并删除指针 → 只杀 bridge PID 17900（`explorer -> cmd -> claude`
> 父进程链原样保留）→ bat 30s 自愈重启 → 服务端新铸 `env_01QkVpx8cjGKktvNzdEVb1ob`
> + 全新会话 `cse_01TGjnrNutzuYDWxSLMrYe1J`。
> **`verify-remote-403.ps1` 结果：FAIL** —— bridge 存活 203s（PASS），但
> `Heartbeat sent = 0` / `worker 403 = 19` / `auth-fail exit = 1`，死法与复用轮**逐字相同**。
>
> 这已经是**今天第三次**新铸环境后照样 403（15:45 A99j 诞生轮、18:16 EUaFsv 轮、
> 22:40 QkVpx 轮），**新铸环境 0/3 活**；而被指控的 `A99j` 复用轮里**有 7 轮是活的**。
> 相关性方向和假设**正好相反**。全天对照表见「第八阶段」。
>
> 另：假设里作为对照的 `env_019vGsvh`，**全机日志 grep 无此物** ——
> 只在 `~/.claude/history.jsonl` 和本次会话 transcript 里命中，即只存在于那条 prompt 的文本中。
> 本机今天注册过的环境只有 `K3huEE / A99j / EUaFsv / QkVpx` 四个。
>
> 错误原因（第三次犯同一个错）：拿**两轮**观测（22:04 活 / 22:12 死）反推因果，
> 而这两轮的环境 ID 差异本身就没有日志支持。**两点连一条线，第四次了。**
> 注：本条假设其实在本文「已证伪的假设」表里**早已有两行**（指针指向已删环境、换全新环境就好），
> 当时的证据是 18:16 那轮；这次只是又花 5 分钟把它重测了一遍。**下次先查这张表。**
