...
- 模型职责统一 GPT-6 Astra＋经典巨著，沿用 Mac 私仓 AGENTS.md 当前契约；不启用付费回退。
- 无历史证据的概率必须标注「规则情景估计（非回测胜率）」。历史条件胜率须按真实成交/前瞻模拟/历史回放分账，带 n、同策略盈亏比及样本外状态；n<30 不显示百分比。
- 入场区间和止损来自可验证结构，不强制 3–5% 带宽，不为仓位或推荐配额改价。1A/2A 为价值跟踪或准备，只有中央当前 3A 现买可给入场许可。
- 新通知合同保留原始证据、评分与版本；未发送不能记成功，服务端接受不等于用户已读。当前工作流存在并不证明推送时限已经验收。
