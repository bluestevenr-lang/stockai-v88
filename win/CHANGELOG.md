...
- 修正Top3口径：短期、中期、长期分别A股3只＋美股3只＋港股3只，每周期9席，按各市场审核分排序；缺证显示空缺，候补仍各市场最多5只/总计15只。
- 同步核验日线原始资料及摘要，修复全新Windows缺少本地数据库而无法绘制周期轮转/未来曲线的问题；保留原行情日期，不追加模型调用。
- 公司资料、行业市值排名和行业前十按实际安装目录读取，中文JSON统一UTF-8；市场旗帜改为本地矢量图片，兼容Windows字体和HTML过滤。
- 同步后仅重启确认为本安装目录的V88进程，加载新版模块；其他进程不变。已有私有交易与历史数据库不被同步包覆盖。
- 新安装验收增加中美港实际资料与图表检查；数据过期明确提示刷新，不冒充当前预测。

## 2026.09.13.3

- 短期8周、中期8–24周、长期12–36周分别研究Top3，按原审核分排序，入场失败不删除研究价值。
- 候补合并后按每市场最多5只限制，总计不超过15；真实审核分降序，默认折叠。
- 多经典按趋势、企业估值、资本风险、策略实证交叉核验；更新后续审核提示词，旧签名及时效保留。
- 主榜、周度、深度与飞书共用原评分和合同；后台跨平台依赖修复。
- 运行中升级后重新打开V88，以加载新的共享模块。

# Win 侧运行环境变更台账（铁律23·逐条登记）

## 2026-08-29（远程运维方案；Win实机尚未部署验收）

- 新增远程运维约定：Win正式接收、Mac正式远程连接、分层健康检查和脱敏告警；不暴露公网控制端口、不增加模型按量费用、不改变投资逻辑。
- 纠正恢复手册：基础GPT诊断聊天与K3/三方投资认证分阶段恢复；`/new`不替代真实问答，同一应用单接收端规则保留。
- 新增独立健康探针及Windows任务安装材料；日常不调用模型，模型未实测就标未验证。不会自动启用渠道、改模型、重置登录或重启网关。
- 修复投影测试夹具缺失中央`factpack_id`造成的假失败，并补“缺少中央事实包仍失败关闭”回归测试。未放宽生产中央裁决或隐私检查。
- 设计及实际代码均经K3订阅CLI复核通过，代码复核时间2026-08-29 00:49（北京时间）；定向测试68通过、1跳过（Mac无PowerShell）。Win尚未接入，不能混写为已上线。
